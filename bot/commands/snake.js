...
class SnakeGame {
    ...
    startGame(msg) {
        ...
        .setColor('#03ad03')
        .setTitle(`Jogo da Cobrinha - ${msg.author.username}`)
        .setDescription(gameBoardTostring())
        .setTimestamp();

        msg.channel.send({ content: " ", embeds: [embed] }).then(gameMessage => {
            gameMessage.react('⬅️');
            gameMessage.react('⬆️');
            gameMessage.react('⬇️');
            gameMessage.react('➡️');
            ...
            .catch(collected => {
                ...
                .setDescription(`**O jogo foi encerrado por inatividade!**
                **Total de maçãs comidas: **${score}`)
                .setTimestamp()
                gameMessage.edit({ content: " ", embeds: [embed] })
            });
            }
            ...
            const gameOver = () => {
                ...
                .setColor("#03ad03")
                .setTitle(`Game Over - ${msg.author.username}`)
                .setDescription(`**Total de maçãs comidas: **${score}`)
                .setTimestamp()
                gameMessage.edit({ content: " ", embeds: [embed] })
...
module.exports = {
  name: "snake",
  aliases: ['cobrinha', 'jogo-da-cobrinha', 'snake-game'],
  run: async (client, message, args) => {
	  
    new SnakeGame().startGame(message)
  }
}